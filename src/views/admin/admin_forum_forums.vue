<template>
  <div>
    <div
      ...
      :key="category.id"
      class="has-text-centered is-centered"
      v-if="categories"
    >
      <p>{{ category.name }}</p>
      <table class="table is-fullwidth">
        ...
        <tbody>
          <AdminForumRow
            v-for="(forum) in orderedForums(category.forums)"
            :key="forum.id"
...
export default {
  ...
  methods: {
    ...
    onForumCreated(forum) {
      ...
      var cat = this.categories.findIndex(
        category => category.id === forum.category
      );
      this.categories[cat].forums.push(forum);
    },
    deleteForum(forum) {
      this.categories.forEach((category, index) => {
        let idx = category.forums.findIndex(
          foundForum => foundForum.id === forum.id
        );
        if (idx !== -1) {
    ...
    forumUp(forum) {
      this.categories.forEach((category, index) => {
        if (forum.order !== 0) {
          let topForumIDX = category.forums.findIndex(
            foundForum => foundForum.order === forum.order - 1
          );
          let forumIDX = category.forums.findIndex(
            foundForum => foundForum.id === forum.id
          );
    ...
    forumDown(forum) {
      this.categories.forEach((category, index) => {
        if (forum.order !== category.forums.length - 1) {
          let bottomForumIDX = category.forums.findIndex(
            foundForum => foundForum.order === forum.order + 1
          );
          let forumIDX = category.forums.findIndex(
            foundForum => foundForum.id === forum.id
    ...
    updateForum(forum) {
      ...
      this.categories.forEach((category, catIdx) => {
        var idx = category.forums.findIndex(
          foundForum => foundForum.id === forum.id
        );
        console.log(idx);
        if (idx !== -1) {
