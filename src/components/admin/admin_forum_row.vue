<template>
  ...
  <tr v-else>
    <td>
      ...
      <div class="tags">
        <span v-for="(file, index) in newIcon" :key="index" class="tag is-primary">
          {{file.name}}
    ...
    <td>
      <b-input v-model="tempForum.name"></b-input>
    </td>
    <td>
      <b-input v-model="tempForum.description" type="textarea"></b-input>
    </td>
    <td>{{ value.thread_count }}</td>
    <td>
...
export default {
  ...
  methods: {
    ...
    updateForum() {
      this.makePetition(
        Forum.updateForum(this.tempForum.slug, this.tempForum)
      ).then(forum => {
    ...
    deleteForum() {
      this.$buefy.dialog.confirm({
        ...
        hasIcon: true,
        onConfirm: () => {
          this.makePetition(Forum.deleteForum(this.value.slug)).then(
            category => {
