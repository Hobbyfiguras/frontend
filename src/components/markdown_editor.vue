  ...
  <div class="vue-bulma-text-editor" v-cloak>
    ...
    <div class="columns is-gapless">
      <div class="column is-half editor-field">
        ...
        ></textarea>
        <span
          v-if="showMarkdownCounter"
          :class="counterClass('md')"
        >{{ content_markdown.length }} {{ markdownLimit > 0 ? ' / ' + markdownLimit : ''}}</span>
      </div>
...
export default {
  ...
  methods: {
    counterClass(type = "md") {
      ...
      if (
        type === "md" &&
        this.markdownLimit > 0 &&
        this.markdownLimit < this.content_markdown.length
      ) {
    ...
    deleteSelection() {
      if (this.getCurrentSelection().length > 0) {
        ...
        this.content_markdown =
          this.content_markdown.substring(
            0,
            this.$refs.textarea.selectionStart
          ) +
          this.content_markdown.substring(
            this.$refs.textarea.selectionEnd,
            this.content_markdown.length
          );
      }
    },
    getLines() {
    ...
    insertText(text) {
      ...
      this.content_markdown =
        this.content_markdown.substring(0, this.$refs.textarea.selectionStart) +
        text +
        this.content_markdown.substring(
          this.$refs.textarea.selectionStart,
          this.content_markdown.length
    ...
    selectTextBack(aftLength, selectLength) {
      this.selectText(
        this.$refs.textarea.selectionEnd - aftLength - selectLength,
    ...
    doAction(action) {
      ...
      if (action.selectable && selectedText.length > 0) {
        ...
        this.selectTextBack(
          action.content.suffix ? action.content.suffix.length : 0,
