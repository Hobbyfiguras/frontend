      ...
      <div class="modal-card">
        ...
        <div class="modal-card-body">
          ...
          <b-field label="Motivo de baneo">
            <b-input placeholder="Motivo de baneo" v-model="banData.ban_reason"></b-input>
          </b-field>
        </div>
    ...
    <div class="tile is-child notification is-info post-time is-size-7">
      <div class="level">
        <div class="level-left">
          <router-link :to="getPostLinkData(post, post.thread)">{{post.created | timeDiff("from")}}</router-link>
        </div>
      </div>
    </div>
    <article
      class="tile is-child notification is-white thread-content"
      :class="{'is-warning': post.deleted}"
    >
      <div class="post-options" v-if="currentUser">
        <b-dropdown>
          ...
          <template
            v-if="(currentUser.id === post.creator.id && !post.deleted || currentUser.is_staff)"
          >
            <b-dropdown-item ref="editButton" v-if="!editing" @click="startEditing()">
              <b-icon icon="pencil"></b-icon>Editar
            </b-dropdown-item>
            <b-dropdown-item v-else ref="cancelEditButton" @click="cancelEditing()">
              <b-icon icon="pencil"></b-icon>Dejar de editar
            </b-dropdown-item>
            <b-dropdown-item ref="deleteButton" @click="askDeletePost()">
          ...
          <b-dropdown-item
            ref="reportButton"
            @click="askReportPost()"
            v-if="currentUser.id !== post.creator.id"
          >
            <b-icon icon="flag"></b-icon>Reportar
          </b-dropdown-item>
          <b-dropdown-item
            ref="banButton"
            @click="isBanModalActive = true"
            v-if="currentUser.id !== post.creator.id && currentUser.is_staff"
          >
            <b-icon icon="gavel"></b-icon>Banear usuario
          </b-dropdown-item>
          <b-dropdown-item ref="banButton" @click="$emit('onQuote', post)" v-if="allowQuote">
            <b-icon icon="share"></b-icon>Citar post
          </b-dropdown-item>
        </b-dropdown>
      </div>
          ...
          <div class="columns is-multiline is-mobile">
            <div class="column is-12-tablet is-one-third-mobile is-hidden-tablet">
              <figure class="image is-128x128">
                <div class="is-paddingless">
                  <router-link
                    class="user-link"
                    :to="{name: 'profile', params: {username: post.creator.username}}"
                  >
            ...
            <div class="column is-12-tablet">
              <p class="subtitle">
                <router-link
                  class="user-link"
                  :class="{'user-moderator': post.creator.is_staff, 'user-banned': isUserBanned(post.creator)}"
                  :to="{name: 'profile', params: {username: post.creator.username}}"
            ...
            <div class="column is-12-tablet is-hidden-mobile is-flex is-horizontal-center">
              <figure class="image is-128x128">
                <div class="is-paddingless">
                  ...
                  >
                    <img
                      v-if="!isUserBanned(post.creator)"
                      class="is-rounded"
          ...
          <div class="columns">
            <div class="column is-12 content">
              <template v-if="!editing">
                ...
                <div
                  v-for="ban in post.bans"
                  :key="ban.id"
                  class="card notification is-danger"
                >(Este usuario fue baneado por este post ("{{ban.ban_reason}}") - {{ban.banner.username}})</div>
                <div v-if="post.modified && post.modified_by" class="is-size-7">
                  <b-icon icon="pencil" size="is-small"></b-icon>
                  Editado por {{post.modified_by.username}} {{post.modified | timeDiff("from")}}
                </div>
              </template>
              <template v-else>
              ...
              <div class="has-vertically-aligned-content">
                <div class="level is-mobile">
                  <div class="level-left is-vertical">
                    <div
                      class="level-item vote-level-item"
                      v-for="vote in post.votes"
                      :key="vote.id"
                    >
                      <div class="tile is-vertical is-ancestor">
                        <div class="tile">
                          <b-tooltip :label="vote.name">
                            <figure
                              class="image is-32x32 vote-list-item"
                              @click="votePostWithID(vote.id)"
                            >
                              <img :src="vote.icon" />
                            </figure>
                          </b-tooltip>
                        ...
                        <div class="tile is-horizontal-center">
                          <b-tooltip
                            class="user-vote-tooltip"
                            position="is-bottom"
                            :label="getUsersTooltip(vote)"
                          >{{vote.vote_count}}</b-tooltip>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="level-right" v-if="currentUser">
                    <template v-if="!hasVoted">
                      <div
                        class="level-item level-vote"
                        v-if="currentUser.id !== post.creator.id"
                        v-for="vote in orderedVoteTypes"
                        :key="vote.id"
                      >
                        <b-tooltip :label="vote.name">
                    ...
                    <div
                      class="level-item"
                      v-if="hasVoted"
                      :class="[hasError ? 'has-text-danger' : 'has-text-success']"
                    >
                      <b-icon v-if="!hasError" icon="check"></b-icon>
      ...
      <div
        v-else
      >Post eliminado por {{post.modified_by.username}} con el motivo: "{{post.delete_reason}}"</div>
    </article>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    votePostWithID(voteTypeID) {
      this.votePost(
        this.forumSettings.vote_types.find(vote => vote.id === voteTypeID)
      );
    },
    ...
    votePost(voteType) {
      Forum.votePost(this.post.id, voteType.id)
        .then(message => {
          this.hasVoted = true;
          this.error = message.success;
          this.oldPost = JSON.parse(JSON.stringify(this.post));
          var voteIDX = this.oldPost.votes.findIndex(
            vote => vote.id === voteType.id
          );
          if (voteIDX === -1) {
    ...
    askReportPost() {
      this.$buefy.dialog.prompt({
        title: "Reportar post",
        message:
          "Antes de reportar un post asegurate que los motivos sean los correctos, de lo contrario tu cuenta podria resultar sancionada.",
        inputAttrs: {
          placeholder: "Motivo"
        },
    ...
    reportPost(reason) {
      this.$awn.async(
        this.makePetition(Forum.reportPost(reason, this.post.id))
      );
    },
    askDeletePost() {
      this.$buefy.dialog.prompt({
